...
from app.models.schemas import (
    ...
    SatisfactionRequest, SatisfactionResponse, QuestionStats, CategoryStats, DailyStats, KeywordStats, GeneralStats, StatisticsResponse
)
...
from app.services.advanced_logger import advanced_logger
...
from app.services.stats import load_data, categorize_question, extract_keywords, load_satisfaction_data
...
async def ask_question_ultra(question_request: QuestionRequest, request: Request):
    ...
    try:
        result = await multimodal_rag_system.query(
            ...
            provider=question_request.provider,
            top_k=question_request.top_k,
            temperature=question_request.temperature,
            max_tokens=question_request.max_tokens
        )

        # Métriques
        from app.utils.logging import query_counter
        query_counter.labels(provider=question_request.provider.value, status="success").inc()
        
        # Enregistrement CSV asynchrone
        processing_time = (time.time() - start_time) * 1000
        
        # Déterminer le modèle utilisé selon le type de réponse
        model_used = result.get("provider_used", question_request.provider.value)
        if model_used == "predefined_qa":
            model_used = "predefined_qa"
        else:
            model_used = question_request.provider.value
        ...
        # Logging avancé avec informations IP
        advanced_logger.log_ask_request(
            request=request,
            endpoint="/ask-question-ultra",
            response_id=result.get("id", str(uuid.uuid4())),
            question=question_request.question,
            response=result.get("answer", ""),
            processing_time_ms=processing_time,
            additional_data={
                "provider": question_request.provider.value,
                "model_used": model_used,
                "confidence_score": confidence_score,
                "tokens_used": result.get("tokens_used", None),
        ...
        error_message = str(e)
        from app.utils.logging import query_counter
        query_counter.labels(provider=question_request.provider.value, status="error").inc()
        ...
        # Logging avancé pour les erreurs
        advanced_logger.log_ask_request(
            request=request,
            endpoint="/ask-question-ultra",
...
async def ask_question_stream_ultra(question_request: QuestionRequest, request: Request):
    ...
    async def generate_ultra_stream():
        ...
        try:
            ...
            if multimodal_rag_system.predefined_qa:
                predefined_response = multimodal_rag_system.predefined_qa.get_predefined_answer(question_request.question)
                
                if predefined_response:
                    logger.info(f"Réponse prédéfinie trouvée pour streaming: {question_request.question[:50]}...")
                    
                    # Métadonnées initiales pour réponse prédéfinie
                    initial_metadata = {
                        "id": query_id,
                        "provider": "predefined_qa",
                        "enhanced_queries": [question_request.question],
                        "timestamp": datetime.now().isoformat(),
            ...
            llm_provider = OptimizedLLMProvider(question_request.provider)
            enhanced_queries = await multimodal_rag_system.query_enhancer.enhance_query(question_request.question, llm_provider)
            ...
            # Re-ranking
            ranked_results = multimodal_rag_system.reranker.rerank(question_request.question, all_results, top_k=question_request.top_k)
...
RÉPONSE:"""
            ...
            advanced_logger.log_ask_request(
                request=request,
                endpoint="/ask-question-stream-ultra",
                response_id=query_id,
                question=question_request.question,
...
async def ask_multimodal_question(multimodal_request: MultimodalQuestionRequest, request: Request):
    """Question avec recherche multimodale (texte + images)"""

    if not multimodal_request.question.strip():
        raise HTTPException(status_code=400, detail="Question vide")
    ...
    try:
        result = await multimodal_rag_system.multimodal_query(
            query=multimodal_request.question,
            provider=multimodal_request.provider,
            content_types=multimodal_request.content_types,
            top_k=multimodal_request.top_k,
        ...
        csv_logger.log_ask_multimodal_question(
            question=multimodal_request.question,
            images_count=len([ct for ct in multimodal_request.content_types if ct == ContentType.IMAGE]),
            response=result.get("answer", ""),
            response_id=result.get("id"),
            sources=[source.get("content", "")[:100] + "..." for source in result.get("sources", [])],
            confidence_score=result.get("confidence", None),
            processing_time_ms=processing_time,
            tokens_used=result.get("tokens_used", None),
            model_used=multimodal_request.provider.value,
            cache_hit=result.get("cache_hit", None),
        ...
        advanced_logger.log_ask_request(
            request=request,
            endpoint="/ask-multimodal-question",
            response_id=result.get("id", str(uuid.uuid4())),
            question=multimodal_request.question,
            response=result.get("answer", ""),
            processing_time_ms=processing_time,
            additional_data={
                "provider": multimodal_request.provider.value,
                "model_used": multimodal_request.provider.value,
                "confidence_score": result.get("confidence", None),
                "tokens_used": result.get("tokens_used", None),
                "cache_hit": result.get("cache_hit", False),
                "sources_count": len(result.get("sources", [])),
                "content_types": [ct.value for ct in multimodal_request.content_types],
                "images_count": len([ct for ct in multimodal_request.content_types if ct == ContentType.IMAGE]),
                "top_k": multimodal_request.top_k,
                "temperature": multimodal_request.temperature,
                "max_tokens": multimodal_request.max_tokens,
        ...
        # Logging avancé avec informations IP pour les erreurs
        advanced_logger.log_ask_request(
            request=request,
            endpoint="/ask-multimodal-question",
            response_id=str(uuid.uuid4()),
...
async def ask_multimodal_with_image(
    ...
    try:
        ...
        advanced_logger.log_ask_request(
            request=request,
            endpoint="/ask-multimodal-with-image",
            response_id=result.get("id", str(uuid.uuid4())),
            question=question,
            response=result.get("answer", ""),
            processing_time_ms=processing_time,
            additional_data={
                ...
                "temperature": temperature,
                "max_tokens": max_tokens,
                "query_had_image": query_image_pil is not None,
                "query_image_filename": query_image.filename if query_image and query_image.filename else None,
                "query_image_size": f"{query_image_pil.size[0]}x{query_image_pil.size[1]}" if query_image_pil else None,
                "query_image_format": query_image_pil.mode if query_image_pil else None,
                "image_analysis": result.get("image_analysis", None),
...
# Permet de récupérer l'ensemble des statistiques concernant les questions 
@router.get("/statistics", response_model=StatisticsResponse)
async def get_all_statistics(top_n: int = 20):
    """
    Retourne toutes les statistiques en une seule requête
    
    - **top_n**: Nombre d'éléments à retourner pour les tops (défaut: 20)
    """
    df = load_data()
    ...
    date_debut = df['timestamp'].min().strftime('%Y-%m-%d')
    date_fin = df['timestamp'].max().strftime('%Y-%m-%d')
    
    # Calculer la moyenne par jour
    df['date'] = df['timestamp'].dt.date
    days_count = df['date'].nunique()
    moyenne_par_jour = total_questions / days_count if days_count > 0 else 0
    ...
    general_stats = GeneralStats(
        ...
        date_fin=date_fin,
        moyenne_par_jour=round(moyenne_par_jour, 2)
    )
    
    # Top questions
    question_counts = df['question'].value_counts().head(top_n)
    top_questions = [
        QuestionStats(
            question=q,
            count=int(count),
    ...
    # Catégories
    df['categorie'] = df['question'].apply(categorize_question)
    ...
    questions_per_day = df.groupby('date').size().reset_index(name='count')
    daily_evolution = [
        DailyStats(
            date=str(row['date']),
            count=int(row['count'])
        )
        for _, row in questions_per_day.iterrows()
    ]
    ...
    models_distribution = df['model_used'].value_counts().to_dict()
    models_distribution = {str(k): int(v) for k, v in models_distribution.items()}
...
async def get_top_questions(limit: int = 20):
    """
    Retourne les questions les plus fréquemment posées
    
    - **limit**: Nombre de questions à retourner (défaut: 20)
    """
    df = load_data()
    total_questions = len(df)
    
    question_counts = df['question'].value_counts().head(limit)
...
async def get_categories():
    """Retourne la répartition des questions par catégories"""
    df = load_data()
    df['categorie'] = df['question'].apply(categorize_question)
...
async def get_daily_evolution(
    ...
    """
    Retourne l'évolution quotidienne du nombre de questions
    ...
    if date_debut:
        date_debut_obj = pd.to_datetime(date_debut).date()
        df = df[df['date'] >= date_debut_obj]
    
    if date_fin:
        date_fin_obj = pd.to_datetime(date_fin).date()
        df = df[df['date'] <= date_fin_obj]
...
async def get_top_keywords(limit: int = 30):
    ...
    - **limit**: Nombre de mots-clés à retourner (défaut: 30)
    """
    df = load_data()
    ...
    result = [
        {
            "keyword": kw,
            "count": count,
            "percentage": round((count / len(all_keywords)) * 100, 2)
        }
        for kw, count in keyword_counts
    ]
...
async def get_general_stats():
    ...
    # Calculer statistiques de temps de traitement
    ...
    stats = {
        ...
        "unique_questions": df['question'].nunique(),
        "periode": {
            "debut": df['timestamp'].min().strftime('%Y-%m-%d %H:%M:%S'),
            "fin": df['timestamp'].max().strftime('%Y-%m-%d %H:%M:%S'),
            "nombre_jours": days_count
        },
        "moyennes": {
            "questions_par_jour": round(len(df) / days_count, 2) if days_count > 0 else 0,
            "temps_traitement_ms": round(processing_times.mean(), 2) if len(processing_times) > 0 else 0,
            "tokens_utilises": round(df['tokens_used'].dropna().mean(), 2)
        },
        "modeles": df['model_used'].value_counts().to_dict(),
        "taux_cache_hit": round((df['cache_hit'].sum() / len(df)) * 100, 2) if 'cache_hit' in df.columns else 0
    }
    
    return stats


#Statistiques concernant la satisfaction des utilisateurs 
@router.get("/satisfaction-stats", response_class=JSONResponse)
def get_satisfaction_stats():
    ...
    try:
        ...
        if "is_satisfied" not in df.columns:
            raise HTTPException(status_code=400, detail="Colonne 'is_satisfied' manquante dans le fichier")

        # 🔹 Calcul du taux global de satisfaction
        taux_global = round(df["is_satisfied"].mean() * 100, 2)

        #taux d'insatisfaction
        taux_insatisfaction = round((1 - df["is_satisfied"].mean()) * 100, 2)


        # 🔹 Calcul de l'évolution journalière
        evolution = (
            df.groupby(df["timestamp"].dt.date)["is_satisfied"]
            .mean()
            .mul(100)
            .reset_index()
        )
        ...
        evolution.columns = ["date", "taux_satisfaction"]

        # ✅ Convertir les dates en chaînes pour le JSON
        evolution["date"] = evolution["date"].astype(str)

        # Conversion finale
        evolution_records = evolution.to_dict(orient="records")

        # 🔹 Construction de la réponse
        return JSONResponse(
            content={
                "taux_global_satisfaction": taux_global,
                "taux_insatisfaction" : taux_insatisfaction,
                "evolution_journaliere": evolution_records,
            }
        )
